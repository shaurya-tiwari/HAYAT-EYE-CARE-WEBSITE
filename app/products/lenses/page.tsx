...
export default async function LensesPage() {
  ...
  const frames = await fetchCloudinaryImages("LENSES");
      ...
      <main className="flex-1 pt-28 pb-20">
        <div className="container-custom px-5 md:px-8">
          ...
          <div className="mb-10 flex flex-col md:flex-row md:items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-[--text-primary] tracking-tight">Premium Lenses</h1>
              <p className="text-[--text-muted] mt-1 text-sm">
                {frames.length} precision lenses available
              </p>
            </div>
          </div>
          ...
          {frames.length === 0 ? (
            <div className="py-20 text-center text-[--text-muted]">
              <p className="font-medium text-sm">No lenses uploaded yet.</p>
              <p className="text-xs mt-1">Upload images to the LENSES folder on Cloudinary.</p>
            </div>
          ) : (
