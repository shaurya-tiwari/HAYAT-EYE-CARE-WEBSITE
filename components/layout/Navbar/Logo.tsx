...
export default function Logo({ scrolled = true }: LogoProps) {
  return (
    <a href="/#home" className={`flex items-center gap-2 md:gap-3 group ${scrolled ? 'whitespace-normal' : 'whitespace-nowrap'}`} aria-label="Hayat Eye Care home">
      {/* Brand Logo Image */}
      <div className="relative w-12 h-12 sm:w-14 sm:h-14 md:w-14 md:h-14 shrink-0 rounded-full bg-white flex items-center justify-center shadow-md overflow-hidden group-hover:scale-105 transition-transform duration-300">
        <Image
          src="/HAYAT LOGO.svg"
