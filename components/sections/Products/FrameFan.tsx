...
export default function FrameFan({ images }: FrameFanProps) {
  ...
  return (
    ...
    <div className="relative w-full h-[280px] sm:h-[420px] md:h-[520px] flex justify-center items-end overflow-hidden mt-0 md:my-8">
      ...
      {/* 
        The anchor point container.
        Positioned at the very bottom edge. Pushed downwards on all screens so the tops don't clip, but the bottoms get cleanly cut by overflow-hidden.
        Added a continuous spin animation to make the fan revolve entirely.
      */}
      <div
        className="relative w-full h-0 flex justify-center items-center translate-y-4 sm:translate-y-16 md:translate-y-24"
      ...
      >
        {slicedImages.map((img, i) => {
          ...
          return (
            <div
              key={`${img.public_id}-${i}`}
              // Adjusted size on mobile (w-[92px] h-[210px]) for narrower but taller look
              className={`fan-card-${i} absolute bottom-0 w-[92px] h-[210px] sm:w-32 sm:h-[280px] md:w-44 md:h-[380px] bg-[#fdfbf7] p-1 sm:p-2 md:p-3 rounded-[6px] sm:rounded-[8px] md:rounded-[12px] flex flex-col items-center`}
              style={{
            ...
            >
              {/* Responsive Transform injected via inline style to allow different gaps for mobile vs desktop */}
              <style dangerouslySetInnerHTML={{
                __html: `
                .fan-card-${i} {
                  /* Increased angle (30) and increased gap (-12vw - 24px) for mobile as requested */
                  transform: translateX(-50%) rotate(${offset * 30}deg) translateY(calc(-12vw - 24px));
                }
                @media (min-width: 768px) {
                  .fan-card-${i} {
                    /* Original wider angle (26) and larger gap for desktop */
